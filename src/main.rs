...
async fn submit_item(
    ...
    let item = add_item(state, new_item.into_inner()).await?;
    // 保存到文件
...
async fn register_user(
    ...
    let registered_user = add_user(state, user).await?;
    let response = ApiResponse {
...
async fn add_to_collection_endpoint(
    ...
    add_hitokoto_to_collection(state, collection_id, request.hitokoto_uuid).await?;
    Ok(Json(
